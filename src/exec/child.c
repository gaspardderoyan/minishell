...
/*   Updated: 2026/01/07 17:04:58 by mgregoir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_directory(t_cmd *cmd, t_data *data)
{
	...
	if (stat(cmd->cmd_path, &sb) == 0 && S_ISDIR(sb.st_mode))
		error_exit(cmd, data, "Is a directory", CANT_EXEC);
}
...
static void	handle_execve_error(char **env_array, t_cmd *cmd, t_data *data)
{
	...
	if (errno == EACCES)
		error_exit(cmd, data, "Permission denied", CANT_EXEC);
	else if (errno == ENOENT)
		error_exit(cmd, data, "No such file or directory", CMD_NOT_FOUND);
	else
	{
		perror("minishell");
		cleanup_child(data);
...
static void	do_execve(t_cmd *cmd, t_data *data)
{
	...
	if (!cmd->cmd_path)
		error_exit(cmd, data, "command not found", CMD_NOT_FOUND);
	check_directory(cmd, data);
	env_array = env_list_to_array(data->env_list);
	if (!env_array)
		error_exit(cmd, data, "malloc error", 1);
